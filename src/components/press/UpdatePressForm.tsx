...
export default function UpdatePressForm({
  pressNote,
  onUpdate,
}: {
  ...
  const {
    ...
    defaultValues: {
      title: pressNote.titulo,
      image: pressNote.foto,
      url: pressNote.url,
      language: pressNote.descripcionIdioma as "ES" | "EN",
    },
  });

  const onSubmit = (data: PressNoteFormValues) => {
    const updatedPressNote = { ...pressNote, ...data };
    ...
    <div className="w-full max-w-md bg-white p-4 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          ...
          <p className="bg-gray-100 text-gray-500 p-2 rounded">{pressNote.idTipPre}</p>
        </div>
        <div>
        ...
        <div>
          <Label>Idioma</Label>
          <Select onValueChange={(value: "ES" | "EN") => setValue("language", value)} defaultValue={pressNote.descripcionIdioma}>
