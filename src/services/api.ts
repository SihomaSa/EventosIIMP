const simulateFetch = <T>(data: T, delay = 0): Promise<T> => {
  ...
  export const fetchPressNotes = () => simulateFetch(import("./pressNotes").then(m => m.default));
  export const fetchActivities = () => simulateFetch(import("./activities").then(m => m.default));
  export const fetchBullentins = () => simulateFetch(import("./bulletins").then(m => m.default));
