...
export default function Bulletins() {
  ...
  const [selectedPressNote, setSelectedPressNote] =
    useState<PressNoteType | null>(null);
  ...
  const getLanguageCount = useCallback(
    (language: string) => {
      return pressNotes.filter((note) => note.prefijoIdioma === language)
        .length;
    },
    [pressNotes]
  );
  ...
  const filteredPressNotes = useMemo(() => {
    if (!pressNotes) return [];

    return pressNotes.filter((note) => {
      ...
      return (
        (note.titulo && note.titulo.toLowerCase().includes(lowerSearchTerm)) ||
        (note.descripcion &&
          note.descripcion.toLowerCase().includes(lowerSearchTerm)) ||
        (note.prefijoIdioma &&
          note.prefijoIdioma.toLowerCase().includes(lowerSearchTerm))
      );
    });
  }, [pressNotes, searchTerm, activeLanguage]);
    ...
    () => (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="text-gray-500 max-w-md mb-6">
          {searchTerm
            ? "No se encontraron boletines de prensa con ese término de búsqueda"
            : activeLanguage !== "all"
    ...
    <div className="p-0 xl:p-6 flex flex-col">
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6 justify-between items-start md:items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100">
        <div className="relative w-full md:w-72">
          ...
          <Input
            placeholder="Buscar boletines de prensa..."
            value={searchTerm}
        ...
        <div className="flex gap-2 w-full md:w-auto">
          ...
          >
            <Plus size={16} />
            <span>Agregar nuevo boletin</span>
          </Button>
        </div>
      </div>
        ...
        {!loading && (
          <div className="mb-6">
            ...
            >
              ...
              >
                {" "}
                {filteredPressNotes.length === 0 && emptyState}
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {filteredPressNotes.map((pressNote, index) => (
                    <PressCard
                      tipoprensa={2}
                      key={`press-${index}-${pressNote.idPrensa}`}
                      pressNote={pressNote}
      ...
      <div className="mt-4 text-sm text-gray-500 flex justify-between items-center">
        <span>
          {!loading && filteredPressNotes.length > 0
            ? `Mostrando ${filteredPressNotes.length} ${
                filteredPressNotes.length === 1
                  ? "boletin de prensa"
                  : "boletines de prensa"
              }${
                activeLanguage !== "all"
